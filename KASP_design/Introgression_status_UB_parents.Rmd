...
date: "10/14/2020 edited 06/25/2021 to add expected introgression status at KASP SNPs"
output:
  html_document:
    df_print: paged
---
...
```{r}
setwd("~/Desktop/Research/Glaziovii_introgression/Ubiaja_Trials")
...
# Read in IDs for KASP SNPs and uplifting key for IDMS:
```{r}
...
u_trueintsect <- read.csv("~/Desktop/Research/Glaziovii_introgression/Ubiaja_Trials/unique_trueintersect.csv", header=FALSE)
...
UBIDMstat2 <- UBIDMstat %>%
  mutate(dosvalue = ifelse(UBIDMstat$Status == "Glaz", UBIDMstat$value,
      ...
      ifelse(UBIDMstat$Status == "Esc" & UBIDMstat$value == 2, 0,               
             ifelse(UBIDMstat$Status == "Esc" & UBIDMstat$value == 1, 1, NA)))))

```



# NEW June 2021: Find parental status at KASP SNPs:
```{r}
#subset for parents that were actually used:
parentsIDMstat = UBIDMstat2 %>% separate(col=Var1, sep=":", into=c("parent","tag")) %>%
  separate(col=parent, sep="_A", into=c("parent", "tag2")) %>%


#uplift IDM list:
upliftkey$Pos <- upliftkey$v6_pos
UBIDMstat_up = join(UBIDMstat2, upliftkey, by = "Pos", type="left")


UBIDM.KASP = subset(UBIDMstat_up, v7_pos %in% kasp.snps) #only finds one KASP IDM
```
...
UBDoseGlaz <- UBIDMstat_up %>% select(Accession=Var1, SNP=Var2, Pos=v7_pos, dosvalue) #for v7 scale
...
DoseGlazag <- DoseGlazag %>% dplyr::rename(accession=Group.1, IDM.window=Group.2, mean.glazdose = x) 
...
saveRDS(DoseGlazsorted, "~/Desktop/Research/Glaziovii_introgression/KASP_design/Verification_05-2021/DoseGlazsorted.rds")
```
...
ggplot(data = DoseGlazsorted) +
  ...
  scale_x_continuous(name="Chromosome 1 Mbp", limits=c(21000000,40000000), 
                     breaks=seq(2.1e7, 4e7, by = 1e6)) +
...
Find expected glaz status in the IDM window:
```{r}
DoseGlazsorted.kasp.df = data.frame()

for (i in 1:(length(kasp.snps))){
  kasp.i = kasp.snps[i]
  kasp.i.subset = subset(DoseGlazsorted, start < kasp.i & stop >= kasp.i)
  kasp.i.subset$kasp = kasp.i
  DoseGlazsorted.kasp.df = rbind(DoseGlazsorted.kasp.df, kasp.i.subset)
}

saveRDS(DoseGlazsorted.kasp.df, file="~/Desktop/Research/Glaziovii_introgression/KASP_design/Verification_05-2021/expected.DoseGlaz.at.kasps.rds")
```


# With accession names visible: v7 scale with KASP snps drawn on:
```{r}
DoseGlazsorted = readRDS( "~/Desktop/Research/Glaziovii_introgression/KASP_design/Verification_05-2021/DoseGlazsorted.rds")

kasp.snps = read.csv("~/Desktop/Research/Glaziovii_introgression/KASP_design/Verification_05-2021/Intertek_verification_report.csv")[,2]


#tiff("UBclones_intro_status_labeled_kasp_uplifted.tiff", units="px", width=1500, height=2000, res=300)

ggplot(data = DoseGlazsorted) +
  geom_segment(aes(x = start, xend = stop, y = accession, yend = accession, color=DoseCat),
               alpha=0.75, size=0.5) +
  scale_y_discrete(name = "Clones at Ubiaja") +
  scale_x_continuous(name="Chromosome 1 Mbp", limits=c(27000000,40000000), 
                     breaks=seq(2.6e7, 4e7, by = 2e6)) +
  scale_color_manual(values=c("#FFFFFF", "#FFCC66", "#FF9900", "#33FFCC", "#0066FF")) +
  labs(color = str_wrap("Average Glaziovii allele dosage", width=10)) +
  theme(axis.ticks.y=element_blank(), plot.background = element_rect(fill="gray95"),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  ...
  geom_vline(xintercept = kasp.snps[1], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[2], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[3], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[4], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[5], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[6], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[7], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[8], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[9], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[10], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[11], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[12], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[13], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[14], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[15], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[16], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[17], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[18], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[19], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[20], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[21], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[22], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[23], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[24], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[25], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[26], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[27], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[28], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[29], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[30], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[31], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[32], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[33], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[34], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[35], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[36], color="black", size=0.2, linetype="dashed") +
  #geom_vline(xintercept = kasp.snps[37], color="black", size=0.2, linetype="dashed") +
  geom_vline(xintercept = kasp.snps[38], color="black", size=0.2, linetype="dashed")
